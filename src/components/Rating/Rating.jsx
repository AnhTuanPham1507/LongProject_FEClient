...
function RatingViewModal(props) {
    ...
    async function handleCreateComment() {
        setLoading(true)
        try {
            await makeRequest.ratingAPI.create(token, { star, r_user: ratingOrder.r_user._id, comment: content, r_order: ratingOrder._id, r_products: ratingProducts })
    ...
    useEffect(() => {
        if (ratingOrder) {
            const tempRatingProducts = ratingOrder.r_exportOrderDetails.map(detail =>
                detail.r_product._id
            )
            setRatingProducts(tempRatingProducts)
        }
    }, [ratingOrder])
                                ...
                                <div class="blog__details__evaluate">
                                    <div className="row">
                                        ...
                                        <div style={{ display: "flex", justifyContent: "space-between" }} class="col-lg-6 col-md-6">
                                            {
                                                ratingOrder?.r_exportOrderDetails.map(detail =>
                                                    <Form.Check
                                                        type="checkbox"
                                                        label={detail.r_product.name}
                                                        defaultChecked
                                                        onChange={(e) => { handleChangeRatingProducts(detail.r_product._id) }}
                                                    />
                                                )
                                            }
